...
export function brief_summary(memObj, __primaryKey__) {
  ...
  let ligInfo = [];
  if (mmjson.chem_comp.name) ligInfo = ligInfo.concat(tmp.map(x=>mmjson.chem_comp.name[x]));
  if (mmjson.chem_comp.pdbx_synonyms) ligInfo = ligInfo.concat(tmp.map(x=>mmjson.chem_comp.pdbx_synonyms[x]));
...
function calculateMW4BU(pdbxData) {
  ...
  if (assembly_id == null) {
    var sinfo = {}, norInfo = {};
    if (pdbxData.entity_poly && pdbxData.entity_poly.entity_id && pdbxData.entity_poly.pdbx_seq_one_letter_code_can) {
