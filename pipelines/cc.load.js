...
export async function pipeline_exec(config) {
  const rdb_def = yaml.safeLoad(await fsp.readFile(config.pipelines.cc.deffile.replace("${CWD}", config.CWD), 'utf8'));
  var jm = await rdbLoader.init(config, rdb_def, true);
  
  var dir = config.pipelines.cc.data.replace("${CWD}", config.CWD);
  if (! dir.endsWith("/")) dir += "/";
