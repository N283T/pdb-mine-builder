...
from pdbminebuilder.db.connection import get_connection, init_pool

console = Console()
# Default logger (no-op if not configured)
_default_logger = logging.getLogger("pdbminebuilder.loader")
...
def _create_engine(conninfo: str) -> Any:
    ...
    Args:
        conninfo: PostgreSQL connection string in psycopg format
            (e.g. "host=localhost port=5433 dbname=pmb user=pdbj").
